...
var launch = function () {
  ...
  var disp_msg = function (data) {
    ...
    if (myNick) {
      ...
      if (
        ...
        var msg = cryptico.decrypt(data.msg, key),
          disp,
          escaped;
  ...
  var userList = function (cancelEvent) {
    ...
    for (var i in pubKeys) {
      if (pubKeys[i] && pubKeys[i] != pubKey) {
        if (empty)
          $('#list').html(
            '<font color="white">Plugged/</font><font color="gray">Unplugged</font> : '
          );
        empty = false;
    ...
    if (!socket.connected) {
      $('#list').html(
        '<font color="white">Plugged/</font><font color="gray">Unplugged</font> : No network.'
      );
      setTimeout(function () {
        userList(true);
      }, 50);
      ...
      if (emptyList > 50) {
        $('#list').html(
          '<font color="white">Plugged/</font><font color="gray">Unplugged</font> : No other users.'
        );
        emptyList = 0;
      }
      setTimeout(function () {
  ...
  var socket_connect = function () {
    var host =
      $(location).attr('protocol').replace(/^http/, 'ws') +
      '//' +
  ...
  nkt_checkReady = setInterval(function () {
    if (nkt_ready > 2) {
      ...
      setInterval(function () {
        if (myNick)
          socket.emit('ping2', { pubKeySrc: pubKey, nickSrc: btoa(myNick) });
        ticks++;
      }, 500);
      setInterval(function () {
        for (var i in pubKeys)
          if (
            pubKeys[i] &&
            Math.abs(userTicks2[i] - userTicks1[i]) -
              Math.abs(userTicks2[i] - ticks) <
              4 * (-2 - Math.abs(userTicks2[i] - userTicks1[i]))
          ) {
            pubKeys[i] = null;
            userList();
          }
  ...
  this.nicks = function () {
    ...
    if (realNicks[0]) return realNicks;
