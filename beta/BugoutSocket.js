(() => {
    ...
    return builder
      ...
      .then(
        ((addr) => () => {
          ...
          window.nkt.userList[addr].sessionCipher = new libsignal.SessionCipher(
            bobStore,
            ALICE_ADDRESS
          );
  ...
  const decryptMessageFrom = (message, from, cipherType) => {
    ...
    if (window.nkt.userList[from].sessionCipher) {
      return window.nkt.userList[from].sessionCipher
      ...
      .then(() => {
        ...
        if (encryptedBool) {
          for (let i in userList) {
            ...
            if (!userList[i].useSignal) {
              ...
              window.nkt.websocket.emit(window.nkt.websocketEventName, {
                ...
                msgData: bugoutEncrypt(
                  ...
                  bugoutMakePacket(window.nkt.mySwarm, {
                    y: 'm',
                    v: JSON.stringify(msg),
                  })
                ),
                msgTo: i,
                msgFrom: window.nkt.mySwarm.address(),
            ...
            encryptMessageTo(JSON.stringify(msgObj), i)
              .then((ciphertext) => {
                const msg = {
                  msgType: 'encrypted',
                  msgData: ciphertext.body,
          ...
          if (window.nkt.singleSwarmID) window.nkt.mySwarm.send(msgObj);
            ...
            for (let i in userList) {
              ...
              if (userList[i].swarmClient) {
                if (msgObj.msgTo && false) {
                  // pour un destinataire //TODO utile ?
                  try {
  ...
  const handleNewMessageReceived = (data) => {
    if (Object(data) === data) {
      ...
      if (!data.ping) {
        ...
        if (data.fromChannel === 'webrtc') {
          ...
          checkNotAlreadyIn(data, 'resentMessages')
            .then(() => {
    ...
    window.dispatchEvent(
      new CustomEvent('nktincomingdata', {
        detail: { data },
      })
    );
  };
  ...
  const answerPreKeyRequest = (fromAddr, forAddr) => {
    ...
    if (forAddr === window.nkt.mySwarm.address()) {
      //anwser for me
      resilientSend({
        msgType: 'preKey',
  ...
  const sendEncryptedMessage = (str, msgTo) => {
    // msgTo optional, private message
    const cont = window.dispatchEvent(
      new CustomEvent('nktsendingmessage', { detail: str })
    );
    if (!cont) return;
    resilientSend(
      {
  ...
  const sendClearMessage = (str) => {
    if (Object(str) === str && str.pubKeySrc) {
  ...
  const setDebugListeners = () => {
    ...
    window.addEventListener('nktnewpeer', (e) => {
      ...
      if (window.nkt.userList[addr] && window.nkt.userList[addr].wasShown)
        return;
    ...
    resilientSend(
      {
        ...
        msgData: preKeyBundleToString(
          window.nkt.userList[addr].myNewPreKeyBundle || window.nkt.preKeyBundle
        ),
        msgDate: new Date().getTime().toString(),
        msgFrom: window.nkt.mySwarm.address(),
        msgBugoutPk: window.nkt.mySwarm.pk,
    ...
    window.addEventListener('nktincomingdata', (e) => {
      if (e.detail.data.msgType === 'sessionEstablishment') {
        ...
        if (window.nkt.userList[e.detail.data.msgFrom].receivedOrderToEstablish)
          return;
        logIfVerbose(
          'PARSING SESSION ESTABLISHMENT FROM ' + e.detail.data.msgFrom
        );
          ...
          decryptMessageFrom(
            ...
            .then((plaintext) => {
              ...
              if (plaintext === window.nkt.mySwarm.address()) {
                ...
                window.nkt.userList[
                  detail.data.msgFrom
                ].receivedOrderToEstablish = false;
                ...
                generateNewPreKeyBundle(window.nkt.signalStore, 1, 1).then(
                  (preKeyBundle) => {
                    window.nkt.userList[detail.data.msgFrom].preKey = null;
                    window.nkt.userList[
                      detail.data.msgFrom
                    ].myNewPreKeyBundle = preKeyBundle;
